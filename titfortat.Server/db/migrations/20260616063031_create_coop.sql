...
CREATE TABLE public.aggregate_events_01_Coop (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_Coop_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
...
create index ix_01_events_Coop_id on public.events_01_Coop(aggregate_id);
create index ix_01_aggregate_events_Coop_id on public.aggregate_events_01_Coop(aggregate_id);
create index ix_01_snapshot_Coop_id on public.snapshots_01_Coop(aggregate_id);
create index ix_01_snapshot_Coop_aggregate_id_and_id on public.snapshots_01_Coop(aggregate_id, id DESC);
create index ix_01_snapshot_Coop_event_id on public.snapshots_01_Coop(event_id);
create index ix_01_events_Coop_timestamp on public.events_01_Coop("timestamp");
create index ix_01_snapshots_Coop_timestamp on public.snapshots_01_Coop("timestamp");
                                                                                                                                                          
CREATE OR REPLACE FUNCTION insert_01_Coop_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_Coop_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
...
CREATE OR REPLACE FUNCTION insert_md_01_Coop_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text   
...
CREATE OR REPLACE FUNCTION insert_md_01_Coop_aggregate_event_and_return_id_opt_lock(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text,   
